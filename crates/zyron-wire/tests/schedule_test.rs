...
//!
//! Exercises CREATE/DROP/PAUSE/RESUME SCHEDULE through the DDL dispatch path and
//! drives the schedule sweep (run_due_schedules) with a controlled clock to
//! verify a due schedule's body runs, its next_run advances, a paused schedule
//! is skipped, and the row the body inserts becomes visible.
//!
//! Run: cargo test -p zyron-wire --test schedule_test -- --nocapture
...
async fn create_schedule_persists_interval() {
    ...
    exec(
        &server,
        &mut session,
        "CREATE SCHEDULE s EVERY 1 HOURS DO INSERT INTO t (id) VALUES (1)",
    )
    .await;
    ...
    let sched = server
        ...
        .expect("schedule s");
    assert_eq!(sched.interval_secs, Some(3600));
    assert!(sched.cron_expr.is_none());
    assert!(!sched.paused);
...
#[tokio::test]
async fn create_schedule_validates_cron() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(&server, &mut session, "CREATE TABLE t (id INT)").await;
    exec(
        &server,
        &mut session,
        "CREATE SCHEDULE c CRON '0 0 * * *' DO INSERT INTO t (id) VALUES (1)",
    )
    .await;
    let sched = server
        .catalog
        .get_schedule_by_name("c")
        .expect("schedule c");
    assert_eq!(sched.cron_expr.as_deref(), Some("0 0 * * *"));
    ...
    let err = try_exec(
        &server,
        &mut session,
        "CREATE SCHEDULE bad CRON 'not a cron' DO INSERT INTO t (id) VALUES (1)",
    )
    .await
    .expect_err("invalid cron should be rejected");
    assert!(err.to_lowercase().contains("cron"), "unexpected: {err}");
}

#[tokio::test]
async fn due_schedule_runs_body_and_advances() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(&server, &mut session, "CREATE TABLE t (id INT)").await;
    exec(
        &server,
        &mut session,
        "CREATE SCHEDULE s EVERY 1 SECONDS DO INSERT INTO t (id) VALUES (42)",
    )
    .await;

    let report = sweep(&server, FAR_FUTURE).await;
    assert_eq!(report.executed, 1);
    assert_eq!(report.failed, 0);

    // The schedule's run state advanced.
    let sched = server
        .catalog
        .get_schedule_by_name("s")
        .expect("schedule s");
...
async fn paused_schedule_is_skipped() {
    ...
    exec(
        &server,
        &mut session,
        "CREATE SCHEDULE s EVERY 1 SECONDS DO INSERT INTO t (id) VALUES (7)",
    )
    .await;
    exec(&server, &mut session, "PAUSE SCHEDULE s").await;
    assert!(server.catalog.get_schedule_by_name("s").unwrap().paused);

    let report = sweep(&server, FAR_FUTURE).await;
    assert_eq!(report.executed, 0);
    let ids = col_i64(&exec(&server, &mut session, "SELECT id FROM t").await, 0);
    assert!(ids.is_empty(), "paused schedule must not run its body");

    // Resume re-enables it.
    exec(&server, &mut session, "RESUME SCHEDULE s").await;
    assert!(!server.catalog.get_schedule_by_name("s").unwrap().paused);
    let report = sweep(&server, FAR_FUTURE).await;
    assert_eq!(report.executed, 1);
}
...
#[tokio::test]
async fn drop_schedule_removes_it() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(&server, &mut session, "CREATE TABLE t (id INT)").await;
    exec(
    ...
    .await;
    assert!(server.catalog.get_schedule_by_name("s").is_some());
    exec(&server, &mut session, "DROP SCHEDULE s").await;
    assert!(server.catalog.get_schedule_by_name("s").is_none());

    // A sweep after the drop does nothing.
    let report = sweep(&server, FAR_FUTURE).await;
    assert_eq!(report.executed, 0);
}

#[tokio::test]
async fn not_yet_due_schedule_does_not_run() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(&server, &mut session, "CREATE TABLE t (id INT)").await;
    exec(
    ...
    .await;
    // Sweep at time 0 (well before the computed next_run): nothing fires.
    let report = sweep(&server, 1).await;
    assert_eq!(report.executed, 0);
}
