//! Integration tests for SQL scalar user-defined functions.
//!
//! Exercises CREATE/DROP FUNCTION through the DDL dispatch path and calls
//! through the plan + execute pipeline, confirming the binder inlines the
//! function body with the call's arguments substituted for the parameters.
//!
//! Run: cargo test -p zyron-wire --test udf_test -- --nocapture
...
fn first_i64(batches: &[DataBatch]) -> i64 {
    ...
    let c = b.columns.first().expect("column");
...
#[tokio::test]
async fn scalar_function_inlines_expression_body() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(
        &server,
        &mut session,
        "CREATE FUNCTION add1(x INT) RETURNS INT AS 'x + 1 'LANGUAGE SQL",
    )
    .await;
    assert_eq!(
        first_i64(&exec(&server, &mut session, "SELECT add1(5)").await),
        6
    );
}

#[tokio::test]
async fn scalar_function_select_body() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(
        &server,
        &mut session,
        "CREATE FUNCTION dbl(x INT) RETURNS INT AS 'SELECT x * 2 'LANGUAGE SQL",
    )
    .await;
    assert_eq!(
...
#[tokio::test]
async fn scalar_function_multiple_params() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(
        &server,
        &mut session,
        "CREATE FUNCTION addxy(a INT, b INT) RETURNS INT AS 'a + b 'LANGUAGE SQL",
    )
    .await;
    assert_eq!(
        first_i64(&exec(&server, &mut session, "SELECT addxy(3, 4)").await),
        7
    );
}

#[tokio::test]
async fn scalar_function_positional_params() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(
        &server,
        &mut session,
        "CREATE FUNCTION pos(a INT, b INT) RETURNS INT AS 'SELECT $1 - $2 'LANGUAGE SQL",
    )
    .await;
    assert_eq!(
        first_i64(&exec(&server, &mut session, "SELECT pos(10, 3)").await),
        7
    );
}

#[tokio::test]
async fn scalar_function_over_table_column() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    ...
    .await;
    let mut vals = col_i64(
        &exec(&server, &mut session, "SELECT add1(v) FROM t ORDER BY id").await,
        0,
    );
    vals.sort();
...
async fn nested_function_calls() {
    ...
    .await;
    assert_eq!(
        first_i64(&exec(&server, &mut session, "SELECT add1(add1(5))").await),
        7
    );
}

#[tokio::test]
async fn or_replace_redefines_body() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(
        &server,
    ...
    exec(
        &server,
        &mut session,
        "CREATE OR REPLACE FUNCTION f(x INT) RETURNS INT AS 'x + 100 'LANGUAGE SQL",
    )
    .await;
    assert_eq!(
        first_i64(&exec(&server, &mut session, "SELECT f(5)").await),
        105
    );
}

#[tokio::test]
async fn drop_function_removes_it() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(
        &server,
    ...
    .await;
    exec(&server, &mut session, "DROP FUNCTION f").await;
    assert!(
        try_exec(&server, &mut session, "SELECT f(5)")
            .await
            .is_err()
    );
}
...
#[tokio::test]
async fn duplicate_signature_without_replace_errors() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(
        &server,
    ...
    assert!(
        try_exec(
            &server,
            &mut session,
            "CREATE FUNCTION f(x INT) RETURNS INT AS 'x + 2 'LANGUAGE SQL"
        )
        .await
        .is_err()
    );
...
#[tokio::test]
async fn recursive_function_is_rejected() {
    let (server, _schema, _tmp) = create_test_server().await;
    let mut session = new_session();
    exec(
        &server,
        &mut session,
        "CREATE FUNCTION rec(x INT) RETURNS INT AS 'rec(x) 'LANGUAGE SQL",
    )
    .await;
    assert!(
        try_exec(&server, &mut session, "SELECT rec(1)")
            .await
            .is_err()
    );
}
...
async fn drop_missing_if_exists_is_noop() {
    ...
    let mut session = new_session();
    assert!(
        try_exec(&server, &mut session, "DROP FUNCTION IF EXISTS nope")
            .await
            .is_ok()
    );
    assert!(
        try_exec(&server, &mut session, "DROP FUNCTION nope")
            .await
            .is_err()
    );
}
