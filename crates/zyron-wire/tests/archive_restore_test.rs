//! Integration tests for ARCHIVE TABLE and RESTORE TABLE.
//!
//! ARCHIVE exports matching rows to an object-store destination as JSON records
//! and removes them from the table. RESTORE reads an archive back and inserts
//! the rows into a target table. The archive destination uses a relative path
//! under /tmp so the opendal fs operator (rooted at "/") resolves it on every
//! platform.
//!
//! Run: cargo test -p zyron-wire --test archive_restore_test -- --nocapture
...
/// A unique relative archive destination under /tmp for one test.
fn archive_dest(tmp: &tempfile::TempDir, leaf: &str) -> String {
    let unique = tmp
        .path()
        .file_name()
        .map(|s| s.to_string_lossy().into_owned())
        .unwrap_or_else(|| "zyron".to_string());
    format!("file://tmp/zyron_archive_test/{unique}/{leaf}.zylog")
}
...
#[tokio::test]
async fn archive_removes_matching_rows_and_restore_brings_them_back() {
    let (server, _schema, tmp) = create_test_server().await;
    let mut session = new_session();
    let dest = archive_dest(&tmp, "roundtrip");
    ...
    // Archive id <= 2 to the destination; those rows leave the table.
    exec(
        &server,
        &mut session,
        &format!("ARCHIVE TABLE t WHERE id <= 2 TO '{dest}'"),
    )
    .await;
    let mut remaining = col_i64(&exec(&server, &mut session, "SELECT id FROM t").await, 0);
    remaining.sort();
    assert_eq!(remaining, vec![3]);

    // Restore the archive into a fresh table.
    exec(
        &server,
        &mut session,
        "CREATE TABLE t_restored (id INT, name VARCHAR)",
    )
    .await;
    exec(
        &server,
        &mut session,
        &format!("RESTORE TABLE t FROM '{dest}' INTO t_restored"),
    )
    .await;
    let mut restored = col_i64(
        &exec(&server, &mut session, "SELECT id FROM t_restored").await,
        0,
    );
    restored.sort();
...
#[tokio::test]
async fn archive_full_table_then_restore_in_place() {
    let (server, _schema, tmp) = create_test_server().await;
    let mut session = new_session();
    ...
    exec(
        &server,
        &mut session,
        &format!("ARCHIVE TABLE t TO '{dest}'"),
    )
    .await;
    assert_eq!(
    ...
    // Restore back into the same table (no INTO).
    exec(
        &server,
        &mut session,
        &format!("RESTORE TABLE t FROM '{dest}'"),
    )
    .await;
...
#[tokio::test]
async fn archive_dry_run_does_not_remove_rows() {
    let (server, _schema, tmp) = create_test_server().await;
    let mut session = new_session();
    ...
    exec(
        &server,
        &mut session,
        &format!("ARCHIVE TABLE t TO '{dest}' DRY RUN"),
    )
    .await;
    // Dry run leaves every row in place.
    assert_eq!(
        row_count(&exec(&server, &mut session, "SELECT id FROM t").await),
        2
    );
...
#[tokio::test]
async fn archive_unknown_table_errors() {
    let (server, _schema, tmp) = create_test_server().await;
    let mut session = new_session();
    let dest = archive_dest(&tmp, "missing");
    assert!(
        try_exec(
            &server,
            &mut session,
            &format!("ARCHIVE TABLE nope TO '{dest}'")
        )
        .await
        .is_err()
    );
...
#[tokio::test]
async fn restore_point_in_time_is_rejected() {
    let (server, _schema, tmp) = create_test_server().await;
    let mut session = new_session();
    ...
    // A versioned restore from a flat archive is rejected with a clear error.
    assert!(
        try_exec(
            &server,
            &mut session,
            &format!("RESTORE TABLE t FROM '{dest}' TO VERSION AS OF 1")
        )
        .await
        .is_err()
    );
