...
fn documented_as(statement: &Statement) -> &'static str {
    match statement {
        ...
        Statement::CreateChangeStream(_) => "CREATE CHANGE STREAM",
        Statement::AlterChangeStream(_) => "ALTER CHANGE STREAM",
        Statement::DropChangeStream(_) => "DROP CHANGE STREAM",
        Statement::ShowChangeStreams(_) => "SHOW CHANGE STREAMS",
        Statement::ApplyChanges(_) => "APPLY CHANGES",
        Statement::VerifyTable(_) => "VERIFY TABLE",
