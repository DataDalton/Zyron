...
impl WalHasher {
    ...
    /// Mixes the 28-byte header fields using two-lane parallel accumulation.
    ///
    ...
    pub fn write_header_fields(
        ...
        self.0.mix_word_a(txn_id.to_le());
...
mod tests {
    ...
    fn test_one_shot_deterministic() {
        ...
        let c1 = wal_checksum(data, 28);
        let c2 = wal_checksum(data, 28);
        assert_eq!(c1, c2);
    }

    #[test]
    fn test_one_shot_empty() {
        let c = wal_checksum(b"", 28);
        assert_ne!(c, 0, "Empty input should produce non-trivial hash");
    }

    #[test]
    fn test_incremental_matches_one_shot() {
        // Build a fake 28-byte header + 20-byte payload
    ...
    fn test_truncation_detected() {
        ...
        let full = wal_checksum(&data, 28);

        // Every truncation length should produce a different checksum
        for len in 1..100 {
            let truncated = wal_checksum(&data[..len], 28);
    ...
    fn test_zeroed_region_detected() {
        ...
        for start in (0..80).step_by(8) {
            ...
            let zeroed = wal_checksum(&corrupted, 28);
            assert_ne!(
                original, zeroed,
    ...
    fn test_various_payload_sizes() {
        ...
        for size in [
            ...
            let mut data = Vec::with_capacity(28 + size);
            // Fake header
