...
//! Phase 5: Query Executor Validation Tests
//!
//! Integration tests for ZyronDB Phase 5 executor components:
...
// =============================================================================
// Validation Infrastructure (mirrors phase1_test.rs)
// =============================================================================
...
fn write_run_json(metrics: &[MetricRecord], utils: &[UtilRecord]) {
    ...
    let fname = format!("phase5_{}.json", run_id());
...
fn raw_log_file() -> &'static Mutex<std::fs::File> {
    RAW_LOG.get_or_init(|| {
        let dir = benchmark_dir();
        let _ = std::fs::create_dir_all(&dir);
        let fname = format!("phase5_{}.txt", run_id());
        let f = std::fs::OpenOptions::new()
            ...
            .unwrap_or_else(|_| {
                std::fs::OpenOptions::new()
                    ...
                    .write(true)
                    .truncate(true)
                    .open(dir.join("phase5_latest.txt"))
