...
impl SessionManager {
    ...
    ///
    /// Takes no connection limit. How many clients may be connected is decided
    /// at accept, against a ceiling derived from the memory the node measured,
    /// and counting them a second time here could only disagree with it.
    pub fn new(idle_timeout_secs: u32) -> Self {
        Self {
    ...
    /// Registers a new session. Returns an error if the process_id is already
    /// registered. Uses entry_sync for atomic check-and-insert to prevent
    /// TOCTOU races.
    pub fn register(
        &self,
        process_id: i32,
    ...
    /// The live connection ceiling, read from the gauge that enforces it.
...
mod tests {
    ...
    /// The manager keeps no cap of its own. How many clients may connect is
    /// decided once, at accept, against the memory-derived ceiling, and a
    /// second count here could only disagree with the one that is enforced.
    #[test]
    fn registration_is_not_capped_here() {
        let mgr = SessionManager::new(0);
        for i in 0..1_000 {
            mgr.register(i, format!("u{i}"), "db".into())
                .expect("registration is not where connections are limited");
        }
        assert_eq!(mgr.active_count(), 1_000);
        // The ceiling it reports is the gauge's, not one it holds
        assert_eq!(
            mgr.max_connections(),
            zyron_pressure::pressure_control::PressureController::global()
                .connections()
