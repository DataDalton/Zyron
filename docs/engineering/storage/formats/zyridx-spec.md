...
Format version 11.0. The running binary writes 11.0 and reads 11.0 only. An older checkpoint is refused and the index is rebuilt from the write-ahead log instead.

## Overview

A `.zyridx` file is a checkpoint of one B+Tree index: every leaf entry of the tree at a given write-ahead log position, laid out as two fixed-stride columns behind a shared key prefix. Loading a checkpoint rebuilds the leaf chain and the internal levels in memory, so recovery replays only the log written after the checkpoint position.
...
[40, 40 + P)            key prefix, P = prefix_len bytes
[.., + N * S)           key suffix column, one S-byte suffix per entry
[.., + N * W)           value column, one W-byte row locator per entry
last 4 bytes            footer checksum over the whole body
```

With `N` the entry count, `S = key_len - prefix_len` and `W = value_width`, the file size is `40 + P + N * (S + W) + 4`. An empty checkpoint is exactly 44 bytes.
...
| 32 | 2 | u16 | key_len | Length of every key in the file, taken from the first entry of the first leaf |
| 34 | 2 | u16 | prefix_len | Bytes of key shared by every entry and stored once |
| 36 | 2 | u16 | value_width | Stride of the value column, 7 or 17 |
| 38 | 2 | bytes | reserved | Zero |

The header stores no root page and no page count. Both are recomputed when the leaves are rebuilt.

Every key in one file has exactly `key_len` bytes. The format carries no per-entry key length.

## Body

### Key prefix

The writer compares the first key of the first leaf with the last key of the last leaf byte by byte. Because the leaf chain is sorted, those two keys bound every key in the tree, so the bytes they share are shared by every entry. That prefix is written once. It is empty when the tree has a single leaf or when keys are empty.

### Key suffix column

`entry_count` suffixes of `key_len - prefix_len` bytes each, in leaf-chain order, which is ascending key order. There is no offsets table, entry `i` starts at `suffixes_start + i * suffix_len`.

### Value column

`entry_count` row locators of `value_width` bytes each, the raw locator payload copied out of the leaf entry with its tag byte first:
...
Both checksums are `hash32` from the workspace checksum module, the 128-bit AES-lane hash folded to 32 bits. The writer folds the body in as it is written, prefix first, then every suffix span in file order, then every value span in file order, with the streaming `Hasher`. The loader recomputes it in one pass over the body.
...
5. Gather every leaf's entries into the suffix and value columns, in runs of about 4 MiB. Below 32 MiB of body the calling thread writes each column itself. Above that, up to 8 writer threads, one per 4 MiB and never more than the machine has cores, take finished runs from a queue and write them with positioned writes, which is what lets them share one file handle without disturbing each other's position.
...
4. Refuse a kind other than `Checkpoint`, a `header_length` other than 40, and a version other than 11.0.
...
Leaf geometry follows from the entry size `2 + key_len + value_width` and the page size of 16384 bytes. Every rebuilt leaf but the last is full, so leaf `k` holds entries from `k * max_entries_per_page`, and a run of leaves can be rebuilt knowing only its starting leaf index. The rebuild runs on up to half the machine's cores, one run per 64 or more leaves.
...
The registry entry for `Checkpoint` declares a single-version reader window at 11.0, the migration policy eager, the migration reversible, and the binary version gate 0.11.0. Eager here means the next checkpoint rewrites the file in full, which is how every checkpoint is written anyway.

A checkpoint at any other version is refused when the header is parsed:

```text
checkpoint is at format version 10.0, this binary writes and reads 11.0. Upgrade through a release that still reads 10.0 to move the checkpoint forward first
```
...
| Entries per rebuilt leaf | `(16384 - 56) / (2 + key_len + value_width + 4)` | Page size, page and leaf header sizes, slot size. 777 for an 8-byte key and a 7-byte locator |
